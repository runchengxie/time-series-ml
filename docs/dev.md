...
| `tests/test_features.py` | `features.py`：30 个技术指标 |
| `tests/test_labels.py` | `labels.py`：二元标签构建 |
| `tests/test_triple_barrier.py` | `labels.py`：Triple Barrier 3-class 标签 |
| `tests/test_split.py` | 时序切分逻辑 |
| `tests/test_industry.py` | `industry.py`：截面行业中性化 |
| `tests/test_crossval.py` | `crossval.py`：PurgedTimeSeriesSplit + purged_cross_val_score |
| `tests/test_backtest.py` | `backtest.py`：walk_forward 回测基本逻辑 |
...
| `metrics.py` | `evaluate`、`compute_ic_icir`、`compute_factor_ic`、`compute_factor_correlation` |
| `regime.py` | `classify_regime`、`compute_market_proxy` |
| `meta_labeling.py` | `train_meta_model`、`apply_meta_filter` |
| `tracking.py` | MLflow 追踪（需外部服务，不适合单元测试） |

添加测试的优先级：`model.py` > `metrics.py` > `regime.py` > `meta_labeling.py`。
...
- 环境变量：数据湖路径用 `TIME_SERIES_ML_DATA_LAKE_ROOT` 和 `TIME_SERIES_ML_INSTRUMENTS_PATH` 配置，不硬编码。
